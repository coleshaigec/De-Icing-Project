...
% % % end
% 
% function simulationParameterGrids = buildSimulationParameterGrids()
% % BUILDSIMULATIONPARAMETERGRIDSFORDEBUGDES
% % Surgical debug grid to:
% % 1) FORCE cancellations
% % 2) VERIFY delay cost
% % 3) CONFIRM zero-cost baseline
% 
% % ==================================================
% % Policies
% % ==================================================
% policies = struct();
% 
% % k=1 → stress (forces cancellation)
% % k=3 → moderate delay
% % k=8 → near-zero congestion baseline
% policies.kValues = [1, 3, 8];
% 
% % Keep code assumptions intact
% policies.eValues = 1;
% 
% % ==================================================
% % Arrival process scenarios
...
% arrivalProcesses = repmat(templateArrivalProcessStruct, numArrivalProcessScenarios, 1);
% 
% % --- STRESS CASE (should trigger HOT + cancellations) ---
% arrivalProcesses(1) = templateArrivalProcessStruct;
% arrivalProcesses(1).scenarioName = "stressPeak";
% arrivalProcesses(1).lambdaBase = 0.25;
% arrivalProcesses(1).lambdaPeak = 0.6;
% arrivalProcesses(1).t0 = 60;
% arrivalProcesses(1).sigmaLambda = 0.3;
% arrivalProcesses(1).Ca = 1;
% 
% % --- MODERATE CASE (should produce delay but no cancellation) ---
% arrivalProcesses(2) = templateArrivalProcessStruct;
% arrivalProcesses(2).scenarioName = "moderateFlow";
% arrivalProcesses(2).lambdaBase = 0.2;
% arrivalProcesses(2).lambdaPeak = 0.3;
% arrivalProcesses(2).t0 = 60;
% arrivalProcesses(2).sigmaLambda = 1.5;
% arrivalProcesses(2).Ca = 1;
% 
% % --- LIGHT CASE (should produce near-zero cost) ---
% arrivalProcesses(3) = templateArrivalProcessStruct;
% arrivalProcesses(3).scenarioName = "lightFlow";
% arrivalProcesses(3).lambdaBase = 0.1;
% arrivalProcesses(3).lambdaPeak = 0.1;
% arrivalProcesses(3).t0 = 60;
% arrivalProcesses(3).sigmaLambda = 3;
% arrivalProcesses(3).Ca = 1;
% 
% % ==================================================
% % Service process scenarios (DO NOT CHANGE SIZE)
% % ==================================================
% numServiceProcessScenarios = 3;
% templateServiceProcessStruct = buildTemplateServiceProcessStruct();
% serviceProcesses = repmat(templateServiceProcessStruct, numServiceProcessScenarios, 1);
% 
% for i = 1:3
%     serviceProcesses(i) = templateServiceProcessStruct;
% end
% 
% serviceProcesses(1).scenarioName = "baseline";
% serviceProcesses(2).scenarioName = "upgraded";
% serviceProcesses(3).scenarioName = "highEnd";
% 
% % CRITICAL: realistic (not absurd) service rates
% serviceProcesses(1).muDI = 0.15;
% serviceProcesses(2).muDI = 0.20;
% serviceProcesses(3).muDI = 0.30;
% 
% % Keep other parameters unchanged
% for i = 1:3
%     serviceProcesses(i).eta = 0.1;
%     serviceProcesses(i).Cs = 1;
%     serviceProcesses(i).activationCostMultiple = 1;
%     serviceProcesses(i).serviceProcessCAPEXCase = "low";
% end
% 
% % ==================================================
% % Taxi / takeoff (forces HOT violations)
% % ==================================================
% numTaxiTakeoffProcessScenarios = 1;
% templateTaxiTakeoffProcessStruct = buildTemplateTaxiTakeoffProcessStruct();
% taxiTakeoffProcesses = repmat(templateTaxiTakeoffProcessStruct, numTaxiTakeoffProcessScenarios, 1);
% 
% taxiTakeoffProcesses(1) = templateTaxiTakeoffProcessStruct;
% taxiTakeoffProcesses(1).scenarioName = "congestedTaxi";
% taxiTakeoffProcesses(1).beta = 1.5;
% taxiTakeoffProcesses(1).p = 1.5;
% taxiTakeoffProcesses(1).T0 = 10;
% taxiTakeoffProcesses(1).CT = 1;
% 
% % ==================================================
% % Storm parameters (keep tiny for speed)
% % ==================================================
% annualNumberOfStormsValues = [1];
% 
% stormDistributionParameters = struct( ...
%     'alpha', 2, ...
%     'theta', 0.3 ...
...
% 
% costModels(1) = templateCostModelStruct;
% costModels(1).scenarioName = "debugCost";
% 
% % Make delay matter
% costModels(1).delayCosts = [10, 25, 60];
% 
% % Make cancellations REAL
