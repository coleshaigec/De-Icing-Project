...
function simulationParameterGrids = buildSimulationParameterGridsForAnalyticModel()
    % BUILDSIMULATIONPARAMETERGRIDSFORANALYTICMODEL Builds controlled DES credibility grid.
    %
    % EXPERIMENT: FINAL CONTROLLED CREDIBILITY GRID
    %
    % PURPOSE
    %  This grid is designed to replace cancellation-dominated exploratory
    %  sweeps with a defensible final experiment. It emphasizes controlled
    %  comparisons, repeated physical scenarios, and post-processing of cost
    %  assumptions from the same simulated operating histories.
    %
    %  Previous grids swept cost models inside the DES factorial. That made
    %  cost-scenario comparisons vulnerable to Monte Carlo contamination,
    %  because each cost case could be attached to a different random physical
    %  history. This grid therefore includes exactly one DES cost model. The
    %  correct workflow is:
    %
    %      1. Run DES once for each physical scenario in this grid.
    %      2. Export physical statistics and baseline cost components.
    %      3. Recompute alternative accounting/cost scenarios offline from the
    %         same physical statistics.
    %
    % DESIGN QUESTIONS
    %  1. k/e tradeoff: does the system prefer more pads, better service, or
    %     both?
    %  2. Arrival-density sensitivity: does bank concentration matter when
    %     total expected pulse demand is approximately controlled?
    %  3. Taxi/takeoff sensitivity: does downstream variability amplify HOT
    %     recirculation and cancellation risk?
    %  4. Weather exposure: does annual storm frequency change total exposure
    %     without confusing per-storm-day severity effects?
    %
    % CONTROL LOGIC
    %  Arrival-bank scenarios use a Gaussian NHPP pulse. Approximate pulse mass
    %  is proportional to lambdaPeak * sigmaLambda, so lambdaPeak is scaled as:
    %
    %      lambdaPeak = pulseMassProxy / sigmaLambda
    %
    %  This keeps expected pulse demand roughly comparable while changing the
    %  temporal concentration of arrivals.
    %
    % REPLICATION LOGIC
    %  Replications are encoded as separate arrival process scenario names with
    %  identical physical parameters. This assumes the annual runner draws fresh
    %  random variates for each simulation plan. If the runner supports explicit
    %  random seeds, use the replicationIndex field below as the seed offset.
    %
    % EXPECTED GRID SIZE
    %  4 k-values * 3 e-values * (3 sigma-values * 8 reps) arrival scenarios
    %  * 3 taxi scenarios * 2 storm-count values * 1 DES cost model
    %  = 1728 annual simulation plans.
    ...
    % k = 4 is the lower controlled-stress bound.
    % k = 10 is deliberately omitted to spend budget on replications instead
    % of a high-capacity endpoint that previously added little signal.
    policies.kValues = [4, 5, 6, 8];
    ...
    % Arrival process scenarios
    % ==================================================
    sigmaLambdaValues = [0.50, 1.50, 4.00];
    numReplications = 8;
    ...
    % Constant pulse mass proxy.
    % sigmaLambda = 1.50 corresponds to lambdaPeak = 0.16.
    % This gives:
    %   sigma = 0.50 -> lambdaPeak = 0.48, lambdaMax = 0.62
    %   sigma = 1.50 -> lambdaPeak = 0.16, lambdaMax = 0.30
    %   sigma = 4.00 -> lambdaPeak = 0.06, lambdaMax = 0.20
    % The narrow case is a genuine stress test, but the average pulse mass is
    % controlled rather than letting broad banks accidentally carry more demand.
    pulseMassProxy = 0.24;
    ...
    for iSigma = 1:numel(sigmaLambdaValues)
        ...
        for iReplication = 1:numReplications
            ...
            % Optional metadata. Safe even if downstream code ignores it.
            arrivalProcesses(iScenario).replicationIndex = iReplication;
            arrivalProcesses(iScenario).controlledSigmaLambda = sigmaLambda;
            arrivalProcesses(iScenario).controlledPulseMassProxy = pulseMassProxy;
        end
    end
    ...
    % Storm-count and severity parameters
    % ==================================================
    annualNumberOfStormsValues = [8, 16];
    ...
    % DES cost model
    % ==================================================
    % Exactly one DES cost model. Alternative cost scenarios should be applied
    % offline to the same physical statistics to avoid Monte Carlo
    % contamination across accounting assumptions.
    numCostScenarios = 1;
    templateCostModelStruct = buildTemplateCostModelStruct();
    costModels = repmat(templateCostModelStruct, numCostScenarios, 1);

    costModels(1) = templateCostModelStruct;
    costModels(1).scenarioName = "physicalBaselineCost";
    costModels(1).singlePadCAPEX = 20;
    costModels(1).serviceProcessCAPEXes = [5, 10, 20];
    costModels(1).delayCosts = [20, 50, 120];
    costModels(1).baseFluidCost = 2.00;
    ...
    simulationParameterGrids.experimentMetadata = struct();
    simulationParameterGrids.experimentMetadata.experimentName = "finalControlledCredibilityGrid";
    simulationParameterGrids.experimentMetadata.numReplicationsPerSigma = numReplications;
    simulationParameterGrids.experimentMetadata.sigmaLambdaValues = sigmaLambdaValues;
    simulationParameterGrids.experimentMetadata.lambdaBase = lambdaBase;
    simulationParameterGrids.experimentMetadata.pulseMassProxy = pulseMassProxy;
    simulationParameterGrids.experimentMetadata.expectedAnnualPlans = ...
        numel(policies.kValues) ...
        * numel(policies.eValues) ...
        * numel(arrivalProcesses) ...
        * numel(taxiTakeoffProcesses) ...
        * numel(annualNumberOfStormsValues) ...
        * numel(costModels);
end
