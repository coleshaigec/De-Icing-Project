function arrivalEvents = buildAircraftArrivalEventsFromTimestamps(arrivalTimestamps)
    ...
    assert(iscolumn(arrivalTimestamps), ...
    'buildAircraftArrivalEventsFromTimestamps:NonColumnTimestamps', ...
    'arrivalTimestamps must be a column vector.');

    assert(all(isfinite(arrivalTimestamps)), ...
        'buildAircraftArrivalEventsFromTimestamps:InvalidTimestamps', ...
        'arrivalTimestamps must be finite.');
    
    assert(all(arrivalTimestamps >= 0), ...
        'buildAircraftArrivalEventsFromTimestamps:NegativeTimestamps', ...
        'arrivalTimestamps must be nonnegative.');
