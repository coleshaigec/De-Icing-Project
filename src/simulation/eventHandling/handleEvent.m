...
function printEventCalendar(eventCalendar)
    % PRINTEVENTCALENDAR Prints pending event calendar contents.

    if isempty(eventCalendar)
        fprintf('  <empty event calendar>\n');
        return;
    end
    ...
    for idxEvent = 1:numel(eventCalendar)
        event = eventCalendar(idxEvent);
...
function printAircraftArray(aircraftArray)
    ...
    fprintf(['  idx | id | type | loc | arr | DI_q_in | DI_start | ', ...
             'DI_done | TT_start | HOT | cycles | STD | takeoff\n']);
    fprintf(['  ----|----|------|-----|-----|---------|----------|', ...
             '---------|----------|-----|--------|-----|--------\n']);
    ...
    for idxAircraft = 1:numel(aircraftArray)
        ...
        fprintf(['  %3d | %2g | %-4s | %-18s | %7.3f | %7.3f | %8.3f | ', ...
                 '%7.3f | %8.3f | %5.3f | %6g | %7.3f | %7.3f\n'], ...
            idxAircraft, ...
            aircraft.id, ...
