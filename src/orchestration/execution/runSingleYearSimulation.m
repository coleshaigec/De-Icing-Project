function singleYearSimulationResult = runSingleYearSimulation(singleYearSimulationPlan)
    % RUNSINGLEYEARSIMULATION Runs one annual simulation plan.
    %
    % INPUT
    %  singleYearSimulationPlan struct with fields
    ...
    %          .lambdaPeak (nonnegative double)        - peak arrival rate
    ...
    % -- Compute number of storm events to simulate --
    numDayLevelSimulations = numel(singleYearSimulationPlan.stormEvents);

    % -- Preallocate results --
    templateDayLevelSimulationResultStruct = buildTemplateDayLevelSimulationResultStruct();
    dayLevelSimulationResults = repmat(templateDayLevelSimulationResultStruct, numDayLevelSimulations, 1);

    % -- Execute day-level simulations --
    for iStorm = 1:numDayLevelSimulations
        dayLevelSimulationResults(iStorm) = runSingleDaySimulation( ...
            singleYearSimulationPlan, ...
            iStorm);
    end

    % -- Aggregate day-level results over simulation year --
    singleYearSimulationResult = aggregateDayLevelSimulationResults(dayLevelSimulationResults, singleYearSimulationPlan);
end