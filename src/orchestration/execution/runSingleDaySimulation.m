...
% printStructRecursive(singleDaySimulationResult, "  ");
    
end

function printStructRecursive(S, indent)
    if nargin < 2
        indent = ''; % Default indentation for the top level
    end
    ...
    for i = 1:length(fields)
        ...
        % Print the field name with current indentation
        fprintf('%s%s: ', indent, fieldName);
        
        if isstruct(value)
            fprintf('\n'); % Move to next line for the substruct
            % Recursive call with increased indentation
            printStructRecursive(value, [indent, '  ']); 
        else
            % Print non-struct values directly
            disp(value);
        end
    end
end