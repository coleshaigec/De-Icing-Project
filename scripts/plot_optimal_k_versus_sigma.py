...
def main() -> None:
    ...
    if "totalOperatingCost" not in results.columns:
        raise ValueError("CSV must contain totalOperatingCost.")

    optimum_context = ["sigmaLambda", "annualNumberOfStorms"]

    if "taxiTakeoffScenarioName" in results.columns and args.taxi_scenario is None:
        optimum_context.append("taxiTakeoffScenarioName")

    optimum_indices = results.groupby(optimum_context, dropna=False)["totalOperatingCost"].idxmin()
    optimum_results = results.loc[optimum_indices].copy()

    summary = (
        optimum_results
        .groupby(["sigmaLambda", "annualNumberOfStorms"], as_index=False)
        .agg(meanOptimalK=("policyK", "mean"))
        .sort_values(["annualNumberOfStorms", "sigmaLambda"])
    )
    ...
    for i_series, (storm_count, group) in enumerate(summary.groupby("annualNumberOfStorms")):
        ax.plot(
            group["sigmaLambda"],
            group["meanOptimalK"],
            color="black",
            ...
            label=f"{storm_count:g} storm days/year",
        )

    ax.set_xlabel(r"Arrival spread parameter, $\sigma_{\lambda}$")
    ax.set_ylabel("Optimal server count, k")
    ax.set_title("Optimal capacity vs. arrival concentration")
    ax.grid(True)
    ...
    save_figure(fig, resolve_output_directory(args.out_dir), "optimal_k_vs_sigma_lambda")
