...
def main() -> None:
    ...
    required_columns = ["totalDeicingJobs", "totalCancellations"]
    missing_columns = [col for col in required_columns if col not in results.columns]

    if missing_columns:
        raise ValueError(f"Missing required columns: {missing_columns}")

    has_service_time = "totalDeicingServiceTime" in results.columns and "numDays" in results.columns

    if has_service_time:
        total_available_minutes = results["policyK"] * results["numDays"] * 24.0 * 60.0
        results["loadProxy"] = results["totalDeicingServiceTime"] / total_available_minutes
        x_label = "Annual deicing load proxy"
    else:
        results["loadProxy"] = results["totalDeicingJobs"] / results["policyK"]
        x_label = "Annual deicing jobs per server"

    arrival_name_map = {
        "narrowDepartureBank": "Narrow bank",
        "stressNarrowDepartureBank": "Stress/narrow bank",
        "mediumDepartureBank": "Medium bank",
        "broadDepartureBank": "Broad bank",
        "steadyModerate": "Steady flow",
    }
    results["arrivalClass"] = results["arrivalScenarioName"].map(arrival_name_map)
    results["arrivalClass"] = results["arrivalClass"].fillna(results["arrivalScenarioName"])
    ...
    results = results[results["arrivalClass"].isin(display_order)].copy()

    fig, ax = plt.subplots()
    markers = ["o", "s", "^", "D", "v"]

    for i_series, arrival_class in enumerate(display_order):
        group = results[results["arrivalClass"] == arrival_class]
    ...
    ax.set_ylabel("Annual cancellations")
    ax.set_title("Congestion phase transition diagnostic")
    ax.grid(True)
    ...
    ax.text(
        0.02,
        0.98,
        "Upward bend marks cancellation-dominated regime",
    ...
    save_figure(fig, resolve_output_directory(args.out_dir), "phase_transition_cancellations")
